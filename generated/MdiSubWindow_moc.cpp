/****************************************************************************
** Meta object code from reading C++ file 'MdiSubWindow.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.7)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../MdiSubWindow.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'MdiSubWindow.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.7. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_MdiSubWindow[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      14,   13,   13,   13, 0x0a,
      22,   13,   13,   13, 0x0a,
      33,   13,   13,   13, 0x0a,
      50,   13,   13,   13, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_MdiSubWindow[] = {
    "MdiSubWindow\0\0Close()\0CloseAll()\0"
    "CloseAllOthers()\0Replace()\0"
};

void MdiSubWindow::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        MdiSubWindow *_t = static_cast<MdiSubWindow *>(_o);
        switch (_id) {
        case 0: _t->Close(); break;
        case 1: _t->CloseAll(); break;
        case 2: _t->CloseAllOthers(); break;
        case 3: _t->Replace(); break;
        default: ;
        }
    }
    Q_UNUSED(_a);
}

const QMetaObjectExtraData MdiSubWindow::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject MdiSubWindow::staticMetaObject = {
    { &QMdiSubWindow::staticMetaObject, qt_meta_stringdata_MdiSubWindow,
      qt_meta_data_MdiSubWindow, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &MdiSubWindow::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *MdiSubWindow::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *MdiSubWindow::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_MdiSubWindow))
        return static_cast<void*>(const_cast< MdiSubWindow*>(this));
    return QMdiSubWindow::qt_metacast(_clname);
}

int MdiSubWindow::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QMdiSubWindow::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
